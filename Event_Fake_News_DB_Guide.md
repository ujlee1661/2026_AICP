...
`outputs/event.pkl`과 `outputs/fake_new.pkl`의 생성 방식, 데이터 구조,
...
- Injection: 이벤트 주입 기간에만 하루 10개 중 1개를 가짜뉴스로 대체
- 비교 대상: PnL, 수익률, 거래 빈도, 매수/매도 타이밍, 포지션 변화, 판단 로그

중요한 점은 뉴스 개수는 항상 10개로 유지하고, 정보량이 아니라 정보 품질만 오염시키는 것이다.

## 2. 산출물

| 파일 | 역할 |
| --- | --- |
| `outputs/event.pkl` | 삼성전자 주요 이벤트 DB |
| `outputs/fake_new.pkl` | 이벤트 윈도우별 가짜뉴스 주입 DB |
| `outputs/daily_news_selection.csv` | 날짜별 baseline 실제 뉴스 10개 목록 |

`event.pkl`과 `fake_new.pkl`은 실험에서 직접 사용하는 핵심 산출물이다.
`daily_news_selection.csv`는 가짜뉴스가 어떤 실제 뉴스 timestamp/slot을 대체하는지 검증하는 기준 데이터다.
...
| `feed_slot` | baseline 10개 뉴스 중 오전/오후 feed 위치 |
...
| `replace_target_news_id` | 대체되는 실제 뉴스 ID |
| `replace_target_timestamp` | 대체되는 실제 뉴스 timestamp |
| `baseline_news_position` | baseline 10개 뉴스 중 몇 번째 뉴스였는지 |
...
| event row 수 | 54 |
| 날짜별 fake 중복 | 없음 |
| baseline replacement count | `10 -> 9 real + 1 fake` 통과 |
...
즉, 가짜뉴스는 임의의 시간에 삽입되는 것이 아니라 실제 baseline 뉴스 하나의 timestamp를 그대로 대체한다.
...
각 날짜의 baseline 10개 중 `replace_target_news_id`에 해당하는 실제 뉴스 1개를
`fake_new.pkl`의 fake row로 치환한다.
...
fake = pd.read_pickle("outputs/fake_new.pkl")
...
if not fake_rows.empty:
    ...
    replace_id = fake_row["replace_target_news_id"]

    real_news = real_news[real_news["id"] != replace_id]
    injected = {
        "id": fake_row["synthetic_id"],
        "date": fake_row["date"],
        "time": fake_row["time"],
        "category": "fake_injection",
        "title": fake_row["title"],
        "summary": fake_row["content"],
    }
    real_news = pd.concat([real_news, pd.DataFrame([injected])], ignore_index=True)

real_news = real_news.sort_values(["date", "time"]).head(10)
```
